...
import { contact, hero, servicesCards } from "@/constants";
                ...
                <div className={cn(cardClass, "h-full")}>
                  <div className="flex items-center gap-3">
                    <span
                      aria-hidden="true"
                      className={cn(
                        "inline-flex h-10 w-10 items-center justify-center rounded-xl border border-foreground/10 bg-foreground/10",
                      )}
                    />
                    <h3 className="text-base font-semibold text-foreground">
                      {item.title}
            ...
            <div className="flex flex-col items-start justify-between gap-5 rounded-3xl border border-foreground/10 bg-foreground/5 p-6 sm:flex-row sm:items-center">
              <div>
                <p className="text-sm font-semibold text-foreground/80">
                  {hero.services.pageTitle}
                </p>
                <p className="mt-2 text-sm text-foreground/70">
                  {hero.services.subtitle}
                </p>
              </div>
              {hero.services.ctaHref && hero.services.ctaLabel ? (
                <Button
                  href={hero.services.ctaHref}
                  variant="primary"
                  size="lg"
                >
                  {hero.services.ctaLabel}
                </Button>
              ) : null}
            </div>
          </ScrollReveal>
